...
# We will define the mapping for each file
controller_mapping = {
    # Old : New
...
model_mapping = {
    ...
    "app/Models/Mahasiswa/M_UserMahasiswa.php": "app/Models/Mahasiswa/M_User_Mahasiswa.php",  # wait, two UserMahasiswa? one in root, one in folder. I'll just map both, maybe one overrides or is the only one
...
view_mapping = {
    ...
    "app/Views/auth/login.php": "app/Views/auth/V_Login_Auth.php", # Auth can have suffix or not, user said C_Auth, but for view let's just do V_Login.php or V_Login_Auth.php. The prompt says shared without actor. So V_Login.php. Let's make it V_Login.php
    "app/Views/auth/login_pegawai.php": "app/Views/auth/V_LoginPegawai.php",
    "app/Views/auth/register.php": "app/Views/auth/V_Register.php",
...
# Add a check for views starting with V_ (since we are rewriting view names)
for k, v in list(view_mapping.items()):
    view_mapping[k] = v.replace('.php', '')
