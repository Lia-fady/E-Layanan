            ...
            <div class="page-header-stats">
                ...
                <div class="ph-stat">
                    ...
                    <div>
                        ...
                        <div class="ph-stat-label">Menunggu Reviu</div>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="row align-items-center">
                <div class="col-md-7">
                    ...
                    <div class="text-muted" style="font-size: 0.88rem; margin-top: 4px;">
                        Periode: <?= !empty($penempatan['tgl_mulai']) ? tgl_indo($penempatan['tgl_mulai']) : '-' ?> &ndash; <?= !empty($penempatan['tgl_selesai']) ? tgl_indo($penempatan['tgl_selesai']) : '-' ?>
                        <span class="mx-2 text-black-50">|</span>
                        Status: <strong class="<?= (isset($penempatan['status_penempatan']) && $penempatan['status_penempatan'] == 'BERJALAN') ? 'text-success' : 'text-secondary' ?>"><?= esc($penempatan['status_penempatan'] ?? '-') ?></strong>
                    </div>
                </div>
                ...
                <div class="d-flex gap-2">
                    ...
                    <div>
                        ...
                        <p class="small m-0" style="line-height: 1.5; font-size: 0.8rem;">
                            Logbook dapat diisi setelah permohonan Anda disetujui dan kegiatan telah dimulai.
                        </p>
                    </div>
                </div>
            </div>
                ...
                <div class="card-flat h-100">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 border-bottom pb-3" style="border-color: #edf2f7 !important;">
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center mt-3 mt-md-0">
                            <!-- Tombol Tambah Logbook (muncul jika BERJALAN dan dalam periode waktu magang) -->
                            <?php 
                                $today = date('Y-m-d');
                                $is_active_period = false;
                                if (!empty($penempatan)) {
                                    $tgl_mulai = $penempatan['tgl_mulai'] ?? $today;
                                    $tgl_selesai = $penempatan['tgl_selesai'] ?? $today;
                                    if ($today >= $tgl_mulai && $today <= $tgl_selesai) {
                                        $is_active_period = true;
                                    }
                                }
                            ?>
                            <?php if(!empty($penempatan) && $penempatan['status_penempatan'] == 'BERJALAN' && (!isset($is_log_book) || strtolower($is_log_book) != 'tidak') && $is_active_period): ?>
                    ...
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 gap-3">
                        <span class="text-muted" style="font-size: 0.85rem;">Kelola dan telusuri riwayat laporan harian Anda</span>
                        <div class="d-flex flex-wrap gap-2 align-items-center">
                            <select id="filter-status-lb" class="form-select form-select-sm shadow-none" style="width: 150px; font-size: 0.8rem; border-color: #e2e8f0; color: #64748b; background-color: #f8fafc;">
                                <option value="">Semua Status</option>
                                <option value="Menunggu Reviu">Menunggu Reviu</option>
                            ...
                            <div style="position: relative;">
                                ...
                                <input type="text" id="dt-search-lb" class="form-control form-control-sm shadow-none" placeholder="Cari kegiatan..." style="padding-left: 28px; border-radius: 8px; width: 200px; font-size: 0.8rem;">
                            </div>
                        </div>
                    </div>
                                        ...
                                        <td class="text-center">
                                            ...
                                            <?php else: ?>
                                                <span class="badge-status-pending mb-2 d-inline-block">
                                                    <i class="bi bi-hourglass-split me-1"></i> Menunggu Reviu
                                                </span>
                                            <?php endif; ?>
            ...
            <div class="modal-body p-4 bg-white">
                <form action="<?= base_url('mahasiswa/simpanLogbook') ?>" method="POST" enctype="multipart/form-data" onsubmit="event.preventDefault(); var form = this; Swal.fire({title: 'Simpan Laporan Hari Ini?', text: 'Pastikan uraian aktivitas yang Anda tulis sudah lengkap dan sesuai.', icon: 'question', showCancelButton: true, confirmButtonColor: '#0a1d37', cancelButtonColor: '#6c757d', confirmButtonText: 'Ya, Simpan', cancelButtonText: 'Periksa Lagi'}).then((res) => { if(res.isConfirmed) { form.submit(); } });">
                    ...
                    <div class="mb-3">
                        ...
                        <?php
                            $today = date('Y-m-d');
                            $minDate = $penempatan['tgl_mulai'];
                            $maxDate = $penempatan['tgl_selesai'];
                            
                            // Mencegah mahasiswa memilih tanggal di masa depan (anti-cheat)
                            if ($today < $maxDate) {
                                $maxDate = $today;
                            }
                            $defaultDate = $today;
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="logbook_magang_text" name="logbook_magang" rows="5" placeholder="Contoh: Membantu memvalidasi dokumen administrasi pengajuan dinas dan merapikan struktur file template..." required minlength="20"></textarea>
                        <div class="d-flex justify-content-between align-items-center mt-1">
                            <small class="text-muted" id="char-counter">0 karakter</small>
                            <small class="text-secondary" style="font-size: 0.75rem;">(Minimal 20 karakter)</small>
                        </div>
                    </div>
                    <div class="mb-4">
...
$(document).ready(function() {
    $('#logbook_magang_text').on('input', function() {
        var len = $(this).val().length;
        $('#char-counter').text(len + ' karakter');
        if(len < 20) {
            $('#char-counter').removeClass('text-success').addClass('text-danger');
        } else {
            $('#char-counter').removeClass('text-danger').addClass('text-success');
        }
    });
