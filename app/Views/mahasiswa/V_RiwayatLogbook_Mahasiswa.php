...
<?= $this->section('content') ?>
        ...
        <div class="card-flat mb-4" style="padding: 20px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 12px;">
            <div class="row align-items-center">
                <div class="col-md-7">
                    <h6 class="fw-bold text-dark mb-1" style="font-size: 1.05rem;"><i class="bi bi-briefcase-fill text-primary me-2"></i> <?= esc($penempatan['nama_jenis'] ?? 'Kegiatan Magang') ?></h6>
                    <div class="text-muted" style="font-size: 0.85rem;">
                        <i class="bi bi-calendar-range me-1"></i> Periode: <?= date('d M Y', strtotime($penempatan['tgl_mulai'])) ?> &ndash; <?= date('d M Y', strtotime($penempatan['tgl_selesai'])) ?>
                        <span class="mx-2 text-black-50">|</span>
                        <i class="bi bi-record-circle me-1"></i> Status: <strong class="<?= $penempatan['status_penempatan'] == 'BERJALAN' ? 'text-success' : 'text-secondary' ?>"><?= esc($penempatan['status_penempatan']) ?></strong>
                    </div>
                </div>
                <?php if (count($semuaPenempatan) > 1): ?>
                <div class="col-md-5 mt-3 mt-md-0">
                    <label class="form-label mb-1 fw-bold text-dark" style="font-size: 0.75rem;"><i class="bi bi-clock-history me-1"></i>Pilih Riwayat Kegiatan:</label>
                    <select class="form-select shadow-none border-primary" style="background-color: #fff;" onchange="window.location.href='<?= base_url('mahasiswa/logbook') ?>?id_penempatan='+this.value">
        ...
        <div class="row g-4">
            <div class="col-12">
                <div class="card-flat h-100">
                    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 border-bottom pb-3" style="border-color: #edf2f7 !important;">
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center mt-3 mt-md-0">
                            ...
                            <form method="GET" action="<?= base_url('mahasiswa/logbook') ?>" class="d-flex gap-2 m-0">
                                <?php if(request()->getGet('id_penempatan')): ?>
                                    <input type="hidden" name="id_penempatan" value="<?= esc(request()->getGet('id_penempatan')) ?>">
                                <?php endif; ?>
                            ...
                            <?php if(request()->getGet('filter_periode') || request()->getGet('filter_status')): ?>
                                <a href="<?= base_url('mahasiswa/logbook') ?><?= request()->getGet('id_penempatan') ? '?id_penempatan='.request()->getGet('id_penempatan') : '' ?>" class="btn btn-sm btn-light border shadow-none" style="color: #dc2626; background-color: #fff5f5;" title="Reset Filter"><i class="bi bi-x-lg"></i></a>
                            <?php endif; ?>
                            </form>
                        </div>
                    </div>
                    ...
                    <?php if (isset($pager) && $pager->getPageCount('logbook') > 1): ?>
                        <div class="mt-3">
                            <?= $pager->links('logbook', 'custom_pager') ?>
                        </div>
                    <?php endif; ?>
