    ...
    <div class="card-body">
        <div class="mb-3 d-flex flex-column flex-md-row justify-content-between align-items-md-center">
            ...
            <div class="d-flex align-items-center" style="gap: 10px;">
                <select id="filterStatusCustom" class="form-control form-control-sm custom-select custom-select-sm" style="width: 180px;">
...
<td class="text-center align-middle"><?= esc($row->nim ?? $row->nis ?? '-') ?></td>
                            <td>
                                ...
                                <?php
                                    ...
                                    switch ($row->status_penempatan) {
                                        ...
                                        case 'DIBATALKAN':
                                            ...
                                            break;
                                        default:
                                            $statusText = esc($row->status_penempatan);
                                            break;
                                    }
                                ?>
...
/* Left align Nama column (3rd column) */
...
/* Ensure rows are not overly tall */
.table.dataTable tbody tr { height: auto; }
/* Buttons side‑by‑side in action column */
...
<script>
var flashError = <?= json_encode(session()->getFlashdata('error') ?? '') ?>;
if (flashError) {
    Swal.fire({
        icon: 'error',
        title: 'Kuota Bidang Penuh',
        text: flashError,
...
$(document).ready(function() {
    ...
    $('#filterStatusCustom').on('change', function() {
        tableCustom.column(5).search(this.value).draw();
    });
