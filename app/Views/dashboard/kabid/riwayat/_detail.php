...
<div class="dispo-detail pb-3">
    ...
    <div class="d-flex justify-content-end mt-2 gap-2">
        ...
        <form method="POST" action="<?= base_url('kabid/riwayat/batalkan') ?>" style="display:inline;" onsubmit="event.preventDefault(); var form = this; Swal.fire({title: 'Batalkan Kegiatan?', html: 'Kegiatan pemohon ini akan <b>dibatalkan</b> dari bidang Anda. Aksi ini tidak dapat dikembalikan.<br><br><textarea id=\'swal-catatan\' class=\'swal2-textarea\' placeholder=\'Tuliskan alasan pembatalan (wajib)...\' style=\'margin-top: 10px;\'></textarea>', icon: 'warning', showCancelButton: true, confirmButtonColor: '#dc3545', cancelButtonColor: '#64748b', confirmButtonText: 'Ya, Batalkan', cancelButtonText: 'Kembali', reverseButtons: true, preConfirm: () => { var alasan = document.getElementById('swal-catatan').value; if(!alasan.trim()) { Swal.showValidationMessage('Alasan pembatalan wajib diisi!'); return false; } form.querySelector('[name=catatan_batalkan]').value = alasan; }}).then((result) => { if (result.isConfirmed) form.submit(); });">
            <?= csrf_field() ?>
            <input type="hidden" name="id_penempatan_magang" value="<?= esc($p->id_penempatan_magang) ?>">
