...
class C_DashboardKabid extends BaseController
{
    ...
    public function index(): string
    {
        ...
        if ($kuotaRow) {
            $total_kuota = (int) ($kuotaRow->kuota ?? 0);
        }
        $sisa_kuota = max(0, $total_kuota - $total_berjalan);
        $kuota_label = $sisa_kuota <= 0 ? '🔴 Kapasitas Penuh' : '🟢 Kuota Masih Tersedia';
