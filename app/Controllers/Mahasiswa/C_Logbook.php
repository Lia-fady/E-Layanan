...
class C_Logbook extends C_BaseMahasiswa
{
    ...
    public function simpanLogbook()
    {
        ...
        $penempatan = null;
        foreach ($semuaPenempatan as $p) {
            if ($p['id_penempatan_magang'] == $id_penempatan_post) {
                $penempatan = $p;
                break;
            }
        }
        ...
        if (!$penempatan) {
            return redirect()->back()->with('error', 'Gagal menyimpan! Data penempatan tidak ditemukan.');
        }

        $tgl_logbook = $this->request->getPost('tgl_logbook');
        $tgl_mulai = $penempatan['tgl_mulai'];
        $tgl_selesai = $penempatan['tgl_selesai'];
        $today = date('Y-m-d');

        if ($tgl_logbook < $tgl_mulai || $tgl_logbook > $tgl_selesai) {
            return redirect()->back()->with('error', 'Gagal menyimpan! Tanggal logbook harus berada di dalam periode magang Anda.');
        }
        if ($tgl_logbook > $today) {
            return redirect()->back()->with('error', 'Gagal menyimpan! Anda tidak dapat mengisi logbook untuk tanggal di masa depan.');
        }
