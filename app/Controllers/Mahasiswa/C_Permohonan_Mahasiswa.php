...
class C_Permohonan_Mahasiswa extends C_Base_Mahasiswa
{
    public function permohonan()
    {
        ...
        // Jika ada draf atau revisi berkas yang sedang berjalan, arahkan langsung ke halaman edit/lanjutkan
        if (!empty($stateData['permohonan_aktif'])) {
            $aktif = $stateData['permohonan_aktif'];
            if ($aktif['status_persetujuan'] === 'PERBAIKAN_BERKAS') {
                return redirect()->to(base_url('mahasiswa/permohonan/edit/' . $aktif['id_permohonan_magang']))
                                 ->with('info', 'Sistem mengarahkan Anda ke form perbaikan berkas. Silakan periksa catatan revisi dan perbaiki data Anda.');
            } elseif ($aktif['posting_data'] === 'draft') {
                return redirect()->to(base_url('mahasiswa/permohonan/edit/' . $aktif['id_permohonan_magang']))
                                 ->with('info', 'Sistem mengarahkan Anda ke draft permohonan yang belum terkirim.');
            }
        }
        ...
        return view('mahasiswa/V_FormPermohonan_Mahasiswa', $data);
    }
        ...
        if ($tglMulai && $tglSelesai) {
            ...
            if ($diff->invert || $diff->days < 60) {
                return redirect()->back()->withInput()->with('errors', [
                    'tgl_mulai' => 'Durasi permohonan minimal adalah 2 bulan (60 hari).',
                    'tgl_selesai' => 'Tanggal selesai harus minimal 2 bulan setelah tanggal mulai.'
                ]);
            }
        }
        ...
        if ($fileKtm && $fileKtm->isValid() && !$fileKtm->hasMoved()) {
            ...
            if ($pivotKtm) {
                $db->table('t_file_permohonan_magang')->insert([
                    ...
                    'nama_file'            => $fileKtm->getClientName(),
    ...
    public function editPermohonan($id_permohonan)
    {
        ...
        return view('mahasiswa/V_FormEditPermohonan_Mahasiswa', $data);
    }
