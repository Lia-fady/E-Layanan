...
class M_Dashboard extends Model
{
    ...
    public function getSuperAdminStats()
    {
        ...
        $pegawaiCounts = [];
        $totalPegawai = 0;
        if ($db->tableExists('c_user_pegawai') && $db->tableExists('c_user_group')) {
            $roles = $db->table('c_user_pegawai as up')
                ->select('ug.group, COUNT(up.id_user_pegawai) as total')
                ->join('c_user_group as ug', 'ug.id = up.id_user_group', 'left')
                ->groupBy('ug.group')
                ->get()
                ->getResultArray();
            foreach ($roles as $r) {
                $groupName = $r['group'] ?: 'Unknown';
                $pegawaiCounts[$groupName] = $r['total'];
        ...
        $stats['distribusiPeran'] = [
            ...
            'Sekretariat' => $pegawaiCounts['Sekretariat'] ?? 0,
            'Kepala Bidang' => $pegawaiCounts['Kepala Bidang'] ?? 0,
            'Super Admin' => $pegawaiCounts['Super Admin'] ?? ($pegawaiCounts['Superadmin'] ?? 0)
        ];
